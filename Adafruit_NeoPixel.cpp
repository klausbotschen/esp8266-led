...
#endif
  // extension: the upper 8 bits define the split
  split = t >> 8;
...
extern "C" IRAM_ATTR void espShow(uint8_t pinA, uint8_t *pixels, uint32_t numBytes);
extern "C" IRAM_ATTR void espShow4(uint8_t pinA, uint8_t pinB, uint8_t pinC, uint8_t pinD,
          uint8_t split, uint8_t *pixels, uint32_t numBytes);
#elif defined(ESP32)
extern "C" void espShow(uint8_t pinA, uint8_t *pixels, uint32_t numBytes);
extern "C" void espShow4(uint8_t pinA, uint8_t pinB, uint8_t pinC, uint8_t pinD,
          uint8_t split, uint8_t *pixels, uint32_t numBytes);
#endif // ESP8266
...
#define PERIOD_400 ((int)(2.50 * SCALE + 0.5) - (5 * INST))
  ...
  // ESP8266 show() is external to enforce ICACHE_RAM_ATTR execution
  if (!split) espShow(pin, pixels, numBytes);
  else espShow4(pin, pinB, pinC, pinD, split, pixels, numBytes);
...
/*!
  @brief   Add a pixel's color to the existing setting, using a 32-bit 'packed' RGB or RGBW value.
  @param   n  Pixel index, starting from 0.
  @param   c  32-bit color value. Most significant byte is white (for RGBW
              pixels) or ignored (for RGB pixels), next is red, then green,
              and least significant byte is blue.
*/
void Adafruit_NeoPixel::addPixelColor(uint16_t n, uint32_t c) {
  if (n < numLEDs) {
    uint8_t *p, r = (uint8_t)(c >> 16), g = (uint8_t)(c >> 8), b = (uint8_t)c;
    if (brightness) { // See notes in setBrightness()
      r = (r * brightness) >> 8;
